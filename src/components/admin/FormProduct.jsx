...
const FormProduct = () => {
  ...
  // --- ✅ Logic การกรองข้อมูล (แก้ใหม่ให้ตรงกับ Backend Schema) ---
  const filteredProducts = products.filter((item) => {
    ...
    if (filterSubCatId) {
        // ถ้าเลือก Sub -> เช็คที่ subCategoryId
        matchesCategory = item.subCategoryId === Number(filterSubCatId);
    } else if (filterMainCatId) {
        // ถ้าเลือก Main -> เช็คที่ categoryId (เพราะ Backend เก็บ ID พ่อไว้ที่นี่)
      ...
      <div className="bg-white shadow-md rounded-xl overflow-hidden border border-gray-100 mb-8">
        ...
        <div className="p-6">
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* ✅ เพิ่มช่อง Product URL */}
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                        URL สินค้า (Slug) <span className="text-gray-400 text-xs font-normal"> *ภาษาอังกฤษ/ตัวเลข/ขีดกลาง (เช่น iphone-15-pro)</span>
                    </label>
                    <input
                        className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 outline-none transition bg-gray-50"
                        onChange={handleOnChange}
                        value={form.productUrl}
                        name="productUrl"
                        placeholder="ถ้าเว้นว่าง ระบบจะใช้ ID แทน"
                    />
                </div>
                         ...
                         return (
                            <tr key={item.id} className="hover:bg-gray-50 transition duration-150 group">
                                ...
                                <td className="p-4">
                                    ...
                                    <p className="text-xs text-gray-400">ID: {item.id} {item.productUrl ? `| /${item.productUrl}` : ''}</p>
                                </td>
